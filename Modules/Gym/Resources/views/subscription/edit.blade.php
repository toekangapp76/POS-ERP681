@extends('layouts.app')
@section('title', __('gym::lang.edit_subscription'))
@section('content')
    ...
    <section class="content">
        <div class="row">
            <div class="col-md-8">
                <div class="box box-solid">
                    <div class="box-header">
                        <h3 class="box-title"> @lang('gym::lang.edit_subscription')</h3>
                    </div>
                    <div class="box-body">
                        {!! Form::open([
                            'url' => action([\Modules\Gym\Http\Controllers\SubscriptionController::class, 'update'], ['subscription' => $subscription->id]),
                            'method' => 'PUT',
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="package_id" required class="form-control package_id" id="package_id">
                                    <option disabled value="">@lang('gym::lang.choose_package')</option>
                                    @foreach ($packages as $key => $package)
                                        <option value="{{ $package->id }}" {{ $subscription->gym_package_id == $package->id ? 'selected' : '' }}> {{ $package->name }} -( @format_currency($package->amount) per
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                {!! Form::text('start_date', @format_date($subscription->gym_package_start_date), [
                        ...
                        <div class="col-md-6 end_date">
                            <div class="form-group">
                                ...
                                {!! Form::text('end_date', $subscription->gym_package_end_date ? @format_date($subscription->gym_package_end_date) : 'Lifetime', [
                                    'class' => 'form-control',
                                    'readonly',
                                    'required',
                                    'id' => 'end_date',
                        ...
                        <div class="panel-body">
                            <div class="row">
                                ...
                                <div class="col-xs-6 text-right">
                                    @php
                                        $current_package = $packages->firstWhere('id', $subscription->gym_package_id);
                                    @endphp
                                    <strong class="package_name">{{ $current_package ? $current_package->name : '' }}</strong>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-6">
                                ...
                                <div class="col-xs-6 text-right">
                                    <strong class="package_price">
                                        @php
                                            $package_amount = ($current_package && isset($current_package->amount) && is_numeric($current_package->amount)) ? (float) $current_package->amount : 0;
                                        @endphp
                                        @format_currency($package_amount)
                                    </strong>
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-xs-6 text-right">
                                    <strong class="total_discount_show">@format_currency($subscription->discount_amount ?? 0)</strong>
                                </div>
                            </div>
                            <div class="row">
                                <hr>
                                <div class="col-xs-6">
                                    <div class="form-group">
                                        @php
                                            $discount_percent = 0;
                                            if ($current_package && isset($current_package->amount) && is_numeric($current_package->amount) && floatval($current_package->amount) > 0 && floatval($subscription->discount_amount) > 0) {
                                                $discount_percent = (floatval($subscription->discount_amount) / floatval($current_package->amount)) * 100;
                                            }
                                        @endphp
                                        {!! Form::number('discount_percent', $discount_percent, [
                                            'class' => 'form-control',
                                            'id' => 'discount_percent',
                            ...
                            <input type="hidden" name="package_price" id="package_price" value="{{ $current_package ? $current_package->amount : 0 }}">
                            <input type="hidden" name="total_discount" id="total_discount" value="{{ $subscription->discount_amount ?? 0 }}">
                            <input type="hidden" name="final_total_input" id="final_total_input" value="{{ $subscription->final_total }}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xs-6 text-right">
                                    <strong class="total">@format_currency($subscription->final_total)</strong>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="box-body payment_row">
                            @include('gym::partials.payment_row_form', [
                                ...
                                'payment_line' => $subscription->payment_lines->first() ?? $payment_line,
                            ])
                        </div>
                    </div>
                    <div class="payment_row">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <br />
                                @php
                                    $change_return_amount = $subscription->payment_lines->where('is_return', 1)->sum('amount') ?? 0;
                                @endphp
                                <span class="lead text-bold change_return_span">@format_currency($change_return_amount)</span>
                                {!! Form::hidden('change_return', $change_return_amount, [
                                    'class' => 'form-control change_return input_number',
                                    'required',
                                    'id' => 'change_return',
                                ]) !!}
                                @php
                                    $change_return_payment = $subscription->payment_lines->where('is_return', 1)->first();
                                @endphp
                                @if (!empty($change_return_payment))
                                    <input type="hidden" name="change_return_id" value="{{ $change_return_payment->id }}">
                                @endif
                            </div>
                        </div>
                        <div class="hide payment_row" id="change_return_payment_data">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        @php
                                            $_payment_method = $change_return_payment->method ?? 'cash';
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            {!! Form::select(
                                                ...
                                                $accounts,
                                                $change_return_payment->account_id ?? '',
                            ...
                            @include('gym::partials.payment_type_details', [
                                'payment_line' => $change_return_payment ?? $change_return,
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="pull-right"><strong>@lang('lang_v1.balance'):</strong> <span
                                        class="balance_due">@format_currency($subscription->final_total - $subscription->payment_lines->where('is_return', 0)->sum('amount'))</span></div>
                            </div>
                        </div>
                    </div>
                @endcomponent
            ...
            <div class="col-md-12 text-center">
                <a href="{{ action([\Modules\Gym\Http\Controllers\SubscriptionController::class, 'index']) }}" class="tw-dw-btn tw-dw-btn-secondary tw-dw-btn-lg">@lang('messages.cancel')</a>
...
<script src="{{ asset('js/payment.js?v=' . $asset_v) }}"></script>
    ...
    <script>
        $(document).ready(function() {
            var currentStartDate = '{{ $subscription->gym_package_start_date }}';
            
            $('.date_picker').datetimepicker({
                format: moment_date_format,
                ignoreReadonly: true,
                defaultDate: currentStartDate ? moment(currentStartDate) : moment()
            });
