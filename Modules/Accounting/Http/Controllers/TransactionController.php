...
class TransactionController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            if (request()->input('datatable') == 'gym_subscription') {
                return $this->_allGymSubscriptions();
            }
    ...
    protected function _allGymSubscriptions()
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        return DataTables::of($query)
            ...
            ->editColumn('sub_type', function ($row) {
                if ($row->sub_type == 'hour_topup') {
                    return '<span class="label label-info">' . __('gym::lang.topup_hours') . '</span>';
                }
            ...
            ->editColumn('final_total', function ($row) {
                return '<span class="display_currency final-total" data-currency_symbol="true" data-orig-value="' . $row->final_total . '">' . $this->transactionUtil->num_f($row->final_total, true) . '</span>';
            })
            ...
            ->addColumn('action', function ($row) {
                ...
                $html .= '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-primary dropdown-toggle" data-toggle="dropdown" aria-expanded="false">'.__('messages.actions').' <span class="caret"></span><span class="sr-only">Toggle Dropdown</span></button>';
                $html .= '<ul class="dropdown-menu dropdown-menu-left" role="menu">';
                
                if (auth()->user()->can('accounting.map_transactions')) {
                    $html .= '<li><a href="#" data-href="' . action([\Modules\Accounting\Http\Controllers\TransactionController::class, 'map']) . '?type=gym_subscription&id=' . $row->id . '" class="btn-modal" data-container=".view_modal"><i class="fas fa-link"></i> ' . __('accounting::lang.map_transaction') . '</a></li>';
                }
