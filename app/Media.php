...
class Media extends Model
{
    ...
    public static function uploadFile($file)
    {
        ...
        // Determine max size. Allow image_size_limit for image mimetypes.
        $mime = $file->getMimeType();
        $max_size = in_array($mime, ['image/jpeg', 'image/jpg', 'image/png', 'image/gif']) ? config('constants.image_size_limit') : config('constants.document_size_limit');

        if ($file->getSize() <= $max_size) {
            // sanitize original name
            $original_name = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
            $extension = $file->getClientOriginalExtension();
            $sanitized = preg_replace('/[^A-Za-z0-9\-]/', '-', $original_name);
            $sanitized = preg_replace('/-+/', '-', $sanitized);
            $new_file_name = time().'_'.mt_rand().'_'.trim($sanitized, '-').($extension ? '.'.$extension : '');
    ...
    public static function attachMediaToModel($model, $business_id, $uploaded_files, $request = null, $model_media_type = null)
    {
        if (! empty($uploaded_files)) {
            if (is_array($uploaded_files)) {
                ...
                // delete previous media files if exists (one-to-one relationship)
                $existing_medias = $model->media()->get();
                foreach ($existing_medias as $existing_media) {
                    // remove physical file if exists
                    $media_path = public_path('uploads/media/'.$existing_media->file_name);
                    if (file_exists($media_path)) {
                        @unlink($media_path);
                    }
