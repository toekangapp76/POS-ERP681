...
class Util
{
    ...
    public function uploadFile($request, $file_name, $dir_name, $file_type = 'document')
    {
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            // Use appropriate size limit for images and documents
            $max_size = $file_type == 'image' ? config('constants.image_size_limit') : config('constants.document_size_limit');
            ...
            if ($file->getSize() <= $max_size) {
                ...
                // Save into public/uploads/<dir_name>
                $destination = public_path('uploads/' . $dir_name);
                if (! file_exists($destination)) {
                    mkdir($destination, 0755, true);
                }

                if ($file->move($destination, $new_file_name)) {
